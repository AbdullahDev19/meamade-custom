...
const OnboardingFlow = (props: OnboardingWidgetProps) => {
            ...
            },
            {
              ...
              onNext: (order: Order) => {
                setStepComplete({
                  ...
                  onComplete: () => {
                    if (!location.pathname.startsWith(`/a/order/${order.id}`)) {
                      navigate(`/a/order/${order.id}`)
                    }
                  },
                })
              },
  ...
  // the step is changed to the next one.
  useEffect(() => {
    if (location.pathname.startsWith("/a/order/order_") && isOrderCreateStep && "order" in props) {
      // change to the preview product step
      const currentStepIndex = findStepIndex(currentStep)
